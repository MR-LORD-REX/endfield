...
SLUG_URL_BASE="https://endfieldtools.dev/assets/images/endfield/itemicon/{id}.png"
...
class AssetResolver:
    ...
    def _load_all(self) -> None:
        self.ascension_break_node:     dict = self._load_json("common/ascension_break_node.json")
        self.avatar_icon:              dict = self._load_json("profile/avatar_icon.json")
        self.business_card_bg:         dict = self._load_json("profile/business_card_bg.json")
        self.character_full:           dict = self._load_json("character/character_full.json")
        self.character:                dict = self._load_json("character/character.json")
        self.element:                  dict = self._load_json("common/element.json")
        self.equip_type:               dict = self._load_json("relic/equip_type.json")
        self.formula_suffix:           dict = self._load_json("common/formula_suffix.json")
        self.gender:                   dict = self._load_json("common/gender.json")
        self.node_skill_type:          dict = self._load_json("common/node_skill_type.json")
        self.prop_by_id:               dict = self._load_json("stats/prop_by_id.json")
        self.prop_by_name:             dict = self._load_json("stats/prop_by_name.json")
        self.relic_item:               dict = self._load_json("relic/relic_item.json")
        self.relic_suit:               dict = self._load_json("relic/relic_suit.json")
        self.role:                     dict = self._load_json("character/role.json")
        self.splash_offset:            dict = self._load_json("common/splash_offset.json")
        self.weapon_breakthrough:      dict = self._load_json("weapon/weapon_breakthrough_template.json")
        self.weapon_gem_template:      dict = self._load_json("weapon/weapon_gem_template.json")
        self.weapon_gem_term:          dict = self._load_json("weapon/weapon_gem_term.json")
        self.weapon_skill:             dict = self._load_json("weapon/weapon_skill.json")
        self.weapon_type:              dict = self._load_json("weapon/weapon_type.json")
        self.weapon:                   dict = self._load_json("weapon/weapon.json")
        self.effects_map:              dict = self._load_json("relic/effects_map.json")
        self.name_map:                 dict = self._load_json("common/nameMap.json")
        self.medals:                   dict = self._load_json("profile/medals.json")
        self.slugs:                    dict = self._load_json("factory/slugs.json")
    ...
    def get_fact_slug_by_id(self, id: str) -> Optional[str]:
        return self.slugs.get("itemIdToSlug", {}).get(str(id))
    
    def get_fact_id_by_slug(self, slug: str) -> Optional[str]:
        return self.slugs.get("slugToItemId", {}).get(slug)
    
    def get_fact_item_icon_url(self, id: str) -> str:
        url=SLUG_URL_BASE.format(id=id)
        return url